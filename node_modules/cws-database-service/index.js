...
exports.findContents = function(model, endDate, startDate) {
	var promise = new Promise(function(resolve, reject) {
		...
		if (!startDate && !endDate) {
			...
			model.find({ 'createdAt': { $lse: endDate }})
			...
			model.find({ 'createdAt': { $gt: startDate, $lse: endDate }})
	 			.limit(config.limit).sort({ createdAt: -1 })
	 			.exec(function(err, documents) {
	 				if (err) {
						logger.error(filename, err);
 ...
 exports.find = function(model, startIndex, limit) {
 	var promise = new Promise(function(resolve, reject) {
		...
		if (!startIndex && !limit) {
			model.find()
				.sort({ createdAt: -1 }).limit(config.limit*2).exec(function(err, documents) {
				if (err) {
					logger.error(filename, err);
					reject(new Error(err));
				} else {
			...
			model.find().skip(startIndex).limit(limit)
				.sort({ createdAt: -1 }).exec(function(err, documents) {
				if (err) {
					logger.error(filename, err);
					reject(new Error(err));
				} else {
					...
					console.log(new Date(documents[0].createdAt).toISOString())
				}
			});
		}
	});
...
exports.findLastCreated = function(model, lastCreatedDate, startIndex, limit) {
	var promise = new Promise(function(resolve, reject) {
		if (!model) {
		...
		if (!startIndex && !limit) {
	 		model.find({ 'createdAt': { $ls: lastCreatedDate }})
	 			.sort({ createdAt: -1 }).limit(config.limit).exec(function(err, documents) {
	 				if (err) {
						logger.error(filename, err);
						reject(new Error(err));
					} else {
	 		...
	 		model.find({ 'createdAt': { $gt: lastCreatedDate }})
