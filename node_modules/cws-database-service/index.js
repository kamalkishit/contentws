...
exports.find = function(model) {
 	var promise = new Promise(function(resolve, reject) {
		...
		model.find().sort({ createdAt: -1 }).limit(config.limit)
			.exec(function(err, documents) {
				if (err) {
					logger.error(filename, err);
					reject(new Error(err));
...
exports.findAfterCreated = function(model, lastCreatedDate) {
	var promise = new Promise(function(resolve, reject) {
		if (!model) {
...
exports.findFirstCreated = function(model, startCreatedDate) {
	var promise = new Promise(function(resolve, reject) {
		if (!model) {
 			logger.error(filename, 'findAfterCreated:' + 'model is null');
 			reject(new Error('model is null'));
 		}

 		if (!startCreatedDate) {
 			logger.error(filename, 'findAfterCreated:' + 'startCreatedDate is null');
 			reject(new Error('startCreatedDate is null'));
 		}

 		model.find({ 'createdAt': { $gt: startCreatedDate }})
 			.sort({ createdAt: -1 }).limit(config.limit)
 			.exec(function(err, documents) {
 				if (err) {
					logger.error(filename, err);
	...
	var promise = new Promise(function(resolve, reject) {
		...
		model.findOne(options, function(err, result) {
			if (err) {
				...
				logger.info(filename + ':findOne', result);
				resolve(result);
			}
		});	
	});
